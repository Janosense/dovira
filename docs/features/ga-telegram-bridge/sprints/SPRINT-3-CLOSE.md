...
| Merged to `master`; `master` merged into `kyiv`; both productions deployed by hand — the newest run-log row on each install shows `Розклад · Надіслано` for a morning after the deploy, with the trend line visible in the owner's chat | **open — sprint boundary**. The repo holds the first two parts: the three step merges are on `master`, and `master` is merged into `kyiv` by this close. Nothing in the repo can show the two production deploys or the morning after them |
| The October clock change passes without a shifted report | **open — the date has not come.** 2026-10-25 is five weeks after this close. `SPRINT-3.md` says so itself: this item is ticked by a later `/close-sprint` run once the developer confirms the run-log rows of 2026-10-25/26, and if the sprint is closed before then it is confirmed at the next sprint boundary and noted here. **This note is that record** |
