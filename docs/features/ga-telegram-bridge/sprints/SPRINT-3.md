...
- [x] Merged to `master`; `master` merged into `kyiv`; both productions deployed by hand — the three step merges are on `master` (`7d338fe1`, `b4ce8e03`, `932f2715`) and `master` is merged into `kyiv` (`bc1be64b`), with `bin/check.sh` exit 0 on both; both hand deploys, and the morning report on each install carrying the trend line in the owner's chat — confirmed by user 2026-09-16
- [ ] The October clock change passes without a shifted report — evidence: the run-log rows of 2026-10-25/26 on either production install carry the configured time — carried to the next sprint boundary: 2026-10-25 is five weeks after this close, which is the handling this item's own wording provides; `SPRINT-3-CLOSE.md` records it so the next boundary picks it up
