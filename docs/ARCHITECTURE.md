...
| Search stats (theme feature) | `inc/features/search-stats/` | Feature `search-stats` (`docs/features/search-stats/FEATURE.md`). `bootstrap.php` requires the feature's classes and registers their hooks. In as of Sprint 1 Step 2: `Schema` (`dovira\SearchStats\Schema`) creates and upgrades the table `{prefix}dovira_search_queries` with `dbDelta()` on `after_switch_theme`, and on `init` when the option `dovira_search_stats_db_version` is behind `Schema::VERSION`; it never runs dbDelta on a current install. `Purge` (`dovira\SearchStats\Purge`) registers the daily WP-Cron event `dovira_search_stats_purge` on `init` when it is absent. On that event (registered with no arguments) it deletes the rows older than `apply_filters( 'dovira_search_stats_retention_days', 90 )` days, never fewer than 29 (docs/DATA-MODEL.md → Feature `search-stats`) | write to another feature's data; run dbDelta on every request; delete rows from a public request |
