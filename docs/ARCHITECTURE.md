...
| City popup (theme feature) | `inc/features/city-popup/` | Feature `city-popup` (`docs/features/city-popup/FEATURE.md`). `bootstrap.php` requires the feature's classes and hooks `Dialog::print_on_blog()` and `Dialog::print_config()` on `wp_footer`. In as of Sprint 1 Step 2: `Sites` (`dovira\CityPopup\Sites`, pure) takes the site URL (`home_url()` at the call site) and gives the current city (`kyiv` when the URL contains `kyiv`, else `kharkiv`), the Kharkiv origin (scheme + host without a leading `kyiv.` + port), the Kyiv origin (`kyiv.` + that host) and the cookie domain (the bare Kharkiv host). `Pages` (`dovira\CityPopup\Pages`) holds the Ukrainian slugs `news`, `services`, `contacts`: `is_blog()` is a single `post` or the `news` page in the current language (`get_page_by_path()` + `pll_get_post()`); `targets()` gives `services` and `contacts` (permalinks in the current language) and `service` (the type's rewrite slug under `pll_home_url()`), leaving out whatever does not exist. `Dialog` (`dovira\CityPopup\Dialog`) prints on a blog page one closed `<dialog data-city-popup>` — title, «Харків» / «Київ» buttons (`data-city-popup-choice`, autofocus on the first), × (`data-city-popup-close`, `aria-label`) — with the city, both origins and `data-targets` (JSON) as escaped attributes; its four strings are Polylang strings of the theme's list, keyed in Ukrainian. As of Sprint 1 Step 3, `Dialog::render_config()` prints on every page one `<div hidden data-city-popup-config>` with the cookie domain and `data-days` (`dovira_city_popup_days`, `Dialog::DEFAULT_DAYS` = 90, below 1 falls back), because the header switcher saves the city on every page. The browser side is `source/scripts/features/city-popup/`: `logic.js`, pure (target match, plain-click test, the move to the other install with `city-popup=1`, the cookie decision, the cookie strings, the backdrop point; Vitest in `tests/js/city-popup/`), and `city-popup.js`, the DOM glue that `app.js` imports last inside its own `try`/`catch` (Data flows → "Blog click → city question") | read the `dovira_city` / `dovira_city_dismissed` cookies in PHP or vary any output by them; hold a URL literal of either install; print the dialog off the blog; write another feature's data |
...
On the receiving install, `Fallback` (`template_redirect`) answers a 404 under
the service base that carries `city-popup=1` with a 302 to its services page
in the request's language; a 404 without the marker stays a 404. No PHP reads
either cookie, so every URL returns the same HTML to every visitor and
crawler.
