...
| GA → Telegram bridge (plugin) | `wp-content/plugins/ga-telegram-bridge/` | Standalone plugin, feature `ga-telegram-bridge`: PSR-4 `src/` under namespace `GaTelegramBridge` loaded by its own autoloader, `gatb_` prefix, text domain `ga-telegram-bridge`, zero runtime dependencies, OpenSSL required (checked on activation). In as of Sprint 1 Step 3: `Plugin` (bootstrap, activation guard, option creation), `Settings` (the option `gatb_settings` — defaults, validation, typed getters, constant overrides for the two secrets) and `Admin` (screen Settings → "GA → Telegram", Settings API, `manage_options`). `GoogleAuth` + `GaClient` (Sprint 1 Step 4) sign in to Google and read the Data API. The Telegram client, the report, the message and the schedule follow in Steps 5-8 and Sprint 2 | reference the theme, its functions, options or data; ship its dev `vendor/` (gitignored) |
