...
| City popup (theme feature) | `inc/features/city-popup/` | Feature `city-popup` (`docs/features/city-popup/FEATURE.md`). `bootstrap.php` requires the feature's classes and hooks `Dialog::print_on_blog()` on `wp_footer`. In as of Sprint 1 Step 2: `Sites` (`dovira\CityPopup\Sites`, pure) takes the site URL (`home_url()` at the call site) and gives the current city (`kyiv` when the URL contains `kyiv`, else `kharkiv`), the Kharkiv origin (scheme + host without a leading `kyiv.` + port), the Kyiv origin (`kyiv.` + that host) and the cookie domain (the bare Kharkiv host). `Pages` (`dovira\CityPopup\Pages`) holds the Ukrainian slugs `news`, `services`, `contacts`: `is_blog()` is a single `post` or the `news` page in the current language (`get_page_by_path()` + `pll_get_post()`); `targets()` gives `services` and `contacts` (permalinks in the current language) and `service` (the type's rewrite slug under `pll_home_url()`), leaving out whatever does not exist. `Dialog` (`dovira\CityPopup\Dialog`) prints on a blog page one closed `<dialog data-city-popup>` — title, «Харків» / «Київ» buttons (`data-city-popup-choice`, autofocus on the first), × (`data-city-popup-close`, `aria-label`) — with the city, both origins, the cookie domain, `data-days` (`dovira_city_popup_days`, `Dialog::DEFAULT_DAYS` = 90, below 1 falls back) and `data-targets` (JSON) as escaped attributes; its four strings are Polylang strings of the theme's list, keyed in Ukrainian. Nothing opens it yet (Sprint 1 Step 3) | read the `dovira_city` / `dovira_city_dismissed` cookies in PHP or vary any output by them; hold a URL literal of either install; print the dialog off the blog; write another feature's data |
