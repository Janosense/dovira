...
| Project gate (not a feature) | `bin/check.sh` | PHPCS + PHPStan + PHPUnit in the plugin, `php -l` over the theme, PHPUnit in the theme's `tests/` (its own dev-only Composer project, installed by the gate when `tests/vendor/` is missing); run before every commit (docs/TECH-STACK.md → Check command) | be replaced by an ad-hoc shell chain |
