...
$is_latest_news      = dovira_get_acf_field( 'is_latest_news' );
$grid_columns        = dovira_get_acf_field( 'grid_columns' );

if ( $is_latest_news ) :
	$count_news = dovira_get_acf_field( 'count_news' );
	$news       = get_posts( array(
		'numberposts' => $count_news,
		'post_type'   => 'post'
	) );
else:
	$news = dovira_get_acf_field( 'news' );
endif;
?>
<?php if ( $is_visible || $is_preview ) : ?>
	<!-- NEWS start -->
	<section
		class="section section--mb-<?= $margin_bottom; ?> <?php if ( ! empty( $background_color ) ) : ?>section--with-bg<?php endif; ?> news <?php if ( ! $is_visible ) : ?>is-not-visible<?php endif; ?>"
		...
		<div class="wrapper news__wrapper">
			...
			<?php endif; ?>
			<?php if ( ! empty( $news ) ) : ?>
				<ul class="news__list news__list--<?= $grid_columns; ?>">
