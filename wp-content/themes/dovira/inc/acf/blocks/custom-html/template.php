...
$code               = dovira_get_acf_field( 'code' );
...
<?php if ( $is_visible || $is_preview ) : ?>
	<!-- RICH TEXT start -->
	<section id="<?= $block_id; ?>"
		class="section section--mb-<?= $margin_bottom; ?> <?php if ( ! empty( $background_color ) ) : ?>section--with-bg<?php endif; ?> custom-html <?php if ( ! $is_visible ) : ?>is-not-visible<?php endif; ?>"
		...
		<?php endif; ?>
		<div class="wrapper custom-html__wrapper" <?php if (! empty( $container_max_width )) : ?>style="max-width: <?= $container_max_width; ?>px"<?php endif; ?>>
			<?php if ( ! empty( $heading ) || ! empty( $caption ) ) : ?>
				<header class="section__header section__header--align-<?= $header_text_align; ?>">
					...
					<?php if ( ! empty( $caption ) ) : ?>
						<p class="custom-html__caption caption" style="color: <?= $caption_color; ?>;">
							<?= $caption; ?>
						</p>
					<?php endif; ?>
				</header>
			<?php endif; ?>
			<?php if ( ! empty( $code ) ) : ?>
				<div class="custom-html__content">
					<?= $code; ?>
				</div>
			<?php endif; ?>
			<?php if ( ! empty( $cta ) ) : ?>
